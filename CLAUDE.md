...
**No JavaScript, no framework.** Interactivity (hover states, animations, mobile nav collapse) is CSS-only (`:hover`, `@keyframes`, `@media (max-width: 768px)`). The 3 HomeScope interaction demos autoplay/loop via plain `<video autoplay loop muted playsinline>` — no JS needed. Don't introduce a JS dependency for something that can stay CSS/HTML-only, to keep the site's zero-build-step deployment intact.

**Click-to-enlarge lightbox (all 3 case study write-ups).** Every content image inside `.case-content` on `cov_case_study.html`, `ctr_case_study.html`, and `homescope_case_study.html` (screenshots, diagrams, wireframes, component closeups — not hero banners or the autoplay demo videos) opens a full-screen overlay on click, built with the CSS-only `:target` technique (no JS): the image is wrapped in `<a href="#lb-N" class="lightbox-link">`, followed by a sibling `<div id="lb-N" class="lightbox">` containing a `.lightbox-backdrop` (click to close), a `.lightbox-close` × button (click to close), and the enlarged `<img>`. Shared CSS lives in `styles.css` under "Lightbox". IDs (`lb-1`, `lb-2`, …) only need to be unique within each page. Follow this same pattern for any new content image added to a case study write-up.
